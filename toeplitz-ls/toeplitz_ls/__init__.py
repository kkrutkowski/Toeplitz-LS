...
def _load_library():
    ...
    if not LIB_PATH.exists():
        raise FileNotFoundError(
            ...
            "'make lib-generic' or 'make lib-native' before installing."
        )
...
def _asymptotic_dofs(num_observations, nterms):
    ...
    if d2 <= 0:
        raise ValueError(
            "asymptotic normalization requires more than 2 * nterms + 2 "
            "observations"
        )
    return d1, d2
...
class tlsf:
    ...
    def power(
        ...
        if normalization == "standard":
            return out
        chi2_ref = _numpy_chi2_ref(y_arr, dy_arr)
        return _normalize_numpy_power(
            out, normalization, t_arr.size, nterms, chi2_ref
        )
